...
const MyJourney = () => {
 ...
 const [load, setLoad] = useState(true);
   ...
   const filteredData = year.filter(item => (
    ...
    item.publishedItems.length > 0 ||
    item.filteredEducation.length > 0 ||
    item.filteredAcheivement.length > 0 ||
    item.filteredTraining.length > 0 ||
    item.filteredExperience.length > 0
  ));
  ...
  return (
    <>
      <div className="container p-10 mt-8 shadow-2xl  rounded-xl">
...
{/* End Loader */}
      ...
      style={{ maxHeight: "430px" }}>
        ...
        filteredData.map(yearData => (
          ...
          <div key={yearData._id}  className="">
